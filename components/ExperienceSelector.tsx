...
export function ExperienceSelector() {
  ...
  return (
    <section className="relative min-h-[46rem] overflow-hidden bg-[#123B2E]">
      {panels.map((panel, index) => (
        <Image
          ...
          fill
          sizes="100vw"
          className={`object-cover transition duration-700 ${active === index ? "opacity-55 scale-100" : "opacity-0 scale-105"}`}
        />
      ))}
      <div className="absolute inset-0 bg-gradient-to-r from-[#100D0B] via-[#100D0B]/72 to-transparent" />
      <div className="relative mx-auto grid min-h-[46rem] max-w-7xl items-end gap-10 px-6 py-20 md:grid-cols-[0.8fr_1fr]">
        <div>
